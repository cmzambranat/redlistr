...
translated by: "Carlos Zambrana-Torrelio"
date: "`r Sys.Date()`"
...
---
## 1. Introduccion
Ejemplos de como usar las herramientas que `redlistr` provee estan descritas en esta seccion. Estas herramiento

Aqui proveemos un ejemplo de los using In this vignette, we provide an example of using the tools within `redlistr` to 
assess the spatial criteria of the [IUCN Red List of 
Ecosystems](http://iucnrle.org/). 

estos criterios evaluan el cambio en extension de un ecosistema en tiempo (Criterio A) ademas de evaluar las propiedades geograpficas del tamaño de la distribucion (Criterio B). Ambos criterios requieren de mapas de la distribucion del ecosistema en dos tiempos.

Primero cargaremos las librerias necesarias para la evaluacion. 
...
## 2. Importando  datos
Primero es necesario importar todos los datos espaciales que deseamos analizar. En este 
ejemplo usaremos la distribucion de manglares de la region Norte de Western Port Bay, Victoria,
Australia. Los datos vienen incluidos en la libreria. La primera distribicion es de Giri et al., 
([2011](http://onlinelibrary.wiley.com/doi/10.1111/j.1466-8238.2010.00584.x/abstract)), y representa 
la distribucion de manglares en el 2000. La segunda distribucion es una clasificacion de manglar
generada usando la libreria `XGBoost` y datos de Landsat 8. Esta clasificacion representa 
la distribucion de manglar en 2017. Ambas distribuciones geograficas estan almacenadas como rasters,
la presencia del ecosistema esta representada con un valor de 1.


```{r importar datos de distribucion}
